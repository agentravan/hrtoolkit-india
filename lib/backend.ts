...
export function adminSessionValid(req:Request){const raw=parseCookie(req,"hrt_admin");if(!raw||!process.env.ACTION_SIGNING_SECRET)return false;const [sig,expRaw]=raw.split(".");const exp=Number(expRaw);if(!sig||!Number.isFinite(exp)||Date.now()>exp)return false;return timingSafe(sig,sign("admin|"+exp,process.env.ACTION_SIGNING_SECRET))}
